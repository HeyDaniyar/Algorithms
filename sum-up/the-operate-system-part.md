...
- 特定：
  ...
  - 独立性：不同进程的工作不相互影响（正确性不受影响）通过页表来完成
  - 制约性： 因为访问共享数据，资源或进程同步而产生制约

>问题： 如果是你，怎么样来实现进程管理机制，来实现进程的特点？

### 进程控制快
  - 定义：：操作系统管理控制进程运行所用的信息集合
  - 组成：
    - 进程标示信息，即ID，父进程信息，用户标示
    - 处理器状态信息：保存进程的运行现场信息
      - 用户可见寄存器
      - 控制和状态寄存器
      - 栈指针信息
...
- 生命期：
  ...
  - 进程运行： 内核如何选择一个进程运行
  - 进程等待：进程只能自己堵塞自己
  - 进程唤醒：进程只能被其他进程或操作系统唤醒
  - 进程结束：包括正常退出，错误退出，致命错误，被其他进程杀死
...
- 为什么使用线程？
  为了满足如下需求：实体之间可以并发的执行；实体之间共享相同的地址空间
